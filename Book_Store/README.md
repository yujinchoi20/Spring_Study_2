...
2. 회원 기능 
    1) 회원 등록
      * MemberForm(화면계층과 서비스계층을 명확하게 분리 ) : name, city, street, zipcode
      * MemberController : createForm, create
        * createForm : 입력받은 값을 memberForm 이란 이름으로 createMemberForm.html 에 넘긴다. 
        * create : form으로 입력받은 값을 영속성 컨텍스트로 만든다. 그리고 'redirect:'를 사용하여 홈 화면으로 이동하도록 요청한다.
      * createMemberForm.html
   
    2) 회원 조회
      * MemberController : list
         * list : findMembers() 메서드를 사용하여 모든 회원 정보를 members에 저장한 뒤, memberList.html로 넘겨 화면에 뿌린다. 
      * memberList.html 

<참고>

화면 요구사항이 복잡해지면, 엔티티에 화면을 처리하기 위한 기능이 증가한다.
엔티티는 점점 화면에 종속적으로 변하고, 이러한 화면 기능 때문에 지저분해진 엔티티는 결국 유지보수 하기 어려워진다. 

실무에서 엔티티는 핵심 비즈니스 로직만 가지고 있고, 화면을 위한 로직은 없어야 한다. 화면이나 API에 맞는 폼 객체나 DTO를 사용해야 한다. 
화면이나 API 요구사항을 폼 객체로 처리하고, 엔티티는 최대한 순수하게 유지해야 한다. 
